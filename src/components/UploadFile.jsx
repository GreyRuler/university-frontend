...
import {setConnectionName} from "../reducers/db/slice.js";

export function UploadFile({types}) {
    ...
    const onUpload = async () => {
        ...
        const {data: response} = await axiosClient.post('/upload', data(), {
            headers: {
        ...
        dispatch(setConnectionName(response))
        reloadFiles()
    }

    return (
        <label className='upload-file' htmlFor='upload-file'>
            <input type="file" id='upload-file' accept={types}
                   style={{display: 'none'}} ref={fileRef} onChange={onUpload}/>
            <div className="preview-upload-file">
                <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true"
                     role="img" width="3em" height="3em" viewBox="0 0 24 24">
                    <g fill="currentColor">
                        <path
